...
function RelatoriosPage() {
  ...
  const slotsConcluidos = useMemo(
    () => agenda.filter((s) => s.status !== "cancelado"),
    [agenda],
  );

  const porDia = useMemo(() => {
    const m = new Map<string, number>();
    slotsConcluidos.forEach((s) => m.set(s.data, (m.get(s.data) ?? 0) + s.ocupacao));
  ...
  const porSemana = useMemo(() => {
    ...
    slotsConcluidos.forEach((s) => {
      const k = isoWeek(new Date(`${s.data}T00:00:00`));
      m.set(k, (m.get(k) ?? 0) + s.ocupacao);
    });
    return Array.from(m.entries()).sort((a, b) => a[0].localeCompare(b[0]));
  }, [slotsConcluidos]);
  ...
  const porServico = useMemo(() => {
    const m = new Map<string, number>();
    slotsConcluidos.forEach((s) => m.set(s.servico, (m.get(s.servico) ?? 0) + s.ocupacao));
    return Array.from(m.entries()).sort((a, b) => b[1] - a[1]);
  }, [slotsConcluidos]);
  ...
  const pacientesStats = useMemo(() => {
    ...
    faturamentos.forEach((f) => {
      ...
      if (f.statusFinanceiro === "pago") cur.pago += f.valorAtendimento;
      if (f.statusFinanceiro === "pendente") cur.pendente += f.valorAtendimento;
      finPorPac.set(f.pacienteId, cur);
    });
    return pacientes.map((p) => ({
      paciente: p,
      atendimentos: atendPorPac.get(p.id) ?? p.totalAtendimentos ?? 0,
      pago: finPorPac.get(p.id)?.pago ?? p.totalPago ?? 0,
      pendente: finPorPac.get(p.id)?.pendente ?? p.totalPendente ?? 0,
    }));
  ...
  const pacientesFiltrados = useMemo(() => {
    ...
    const list = q
      ? pacientesStats.filter(
          (s) =>
            s.paciente.nomeCompleto.toLowerCase().includes(q) ||
            s.paciente.telefone?.includes(q) ||
      ...
      <Tabs defaultValue="atendimentos">
        ...
        <TabsContent value="atendimentos" className="space-y-4 mt-4">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <MiniKpi label="Média por dia" value={mediaDia.toFixed(1)} />
            <MiniKpi label="Média por semana" value={mediaSemana.toFixed(1)} />
            <MiniKpi label="Média por mês" value={mediaMes.toFixed(1)} />
          </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              {porServico.length === 0 && <p className="text-sm text-muted-foreground">Sem atendimentos.</p>}
              {porServico.map(([k, v]) => (
                <div key={k}>
        ...
        <TabsContent value="financeiro" className="space-y-4 mt-4">
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              ...
              {inadimplentes.map((s) => (
                <div key={s.paciente.id} className="flex items-center justify-between border rounded-md p-2">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {pacientesFiltrados.length === 0 && (
                      <tr><td colSpan={4} className="p-6 text-center text-muted-foreground">Nenhum paciente encontrado.</td></tr>
                    )}
                    {pacientesFiltrados.map((s) => (
                      <tr key={s.paciente.id} className="border-t">
                        <td className="p-3">
                          <div className="font-medium">{s.paciente.nomeCompleto}</div>
                          <div className="text-xs text-muted-foreground">{s.paciente.telefone ?? "—"}</div>
                        </td>
                        <td className="p-3 text-right tabular-nums">
                          <Badge variant="secondary">{s.atendimentos}</Badge>
                        </td>
                        <td className="p-3 text-right tabular-nums text-emerald-600">{brl(s.pago)}</td>
                        <td className="p-3 text-right tabular-nums">
                          <span className={s.pendente > 0 ? "text-amber-600 font-medium" : "text-muted-foreground"}>
                            {brl(s.pendente)}
                          </span>
                        </td>
                      </tr>
  ...
  return (
    <Card>
      <CardHeader className="flex-row items-center justify-between space-y-0">
        <CardTitle className="text-base flex items-center gap-2">{icone}{titulo}</CardTitle>
